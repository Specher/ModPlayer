...
public class Module {
	...
	public void toStringBuffer( StringBuffer out ) {
		out.append( "歌曲名(Song Name): " + songName + '\n'
			+ "通道数(Num Channels): " + numChannels + '\n'
			+ "乐器数(Num Instruments): " + numInstruments + '\n'
			+ "节奏数(Num Patterns): " + numPatterns + '\n'
			+ "序列长度(Sequence Length): " + sequenceLength + '\n'
			+ "重放位置(Restart Pos): " + restartPos + '\n'
			+ "默认速度(Default Speed): " + defaultSpeed + '\n'
			+ "默认拍子(Default Tempo): " + defaultTempo + '\n'
			+ "线性周期(Linear Periods): " + linearPeriods  );
//		out.append( "序列(Sequence): " );
//		for( int seqIdx = 0; seqIdx < sequence.length; seqIdx++ ){
//			if(sequence.length-1!=seqIdx){
...
//			out.append( "乐器(Instrument) " + insIdx + ":\n" );
//			instruments[ insIdx ].toStringBuffer( out );
//		}
	}
	public StringBuffer getPatterns( ){
	...
	public StringBuffer getInstruments( ){
		StringBuffer out = new StringBuffer();
		for( int insIdx = 1; insIdx < instruments.length; insIdx++ ) {
